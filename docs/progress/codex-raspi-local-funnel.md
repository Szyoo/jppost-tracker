...
- [x] 两个安全问题 —— 2026-07-15 在 `fix/auth-hardening` 分支修复并在 VPS 实机验证通过：
  1. `is_admin()` 改为实时查库（请求级缓存），实测降级后旧 session 立即 403；
  2. 登录限流 IP 改为按 `TRUSTED_PROXY_COUNT` 取 X-Forwarded-For 右数第 N 个受信值，
     实测每次伪造不同转发头仍在第 6 次被锁定。
- [x] 合并回 `main` —— 2026-07-15 已合并（部署分支与安全修复分支均已进 main）。
